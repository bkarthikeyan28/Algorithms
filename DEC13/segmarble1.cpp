...
#include <stdio.h>
#include <math.h>
int getMid(long long int s, long long int e) {  return s + (e -s)/2;  }
...
long long int *constructST(long long int arr[], long long int n)
{
    ...
    long long int x = (int)(ceil(log2(n))); //Height of segment tree
    long long int max_size = 2*(int)pow(2, x) - 1; //Maximum size of segment tree
...
int main()
{
    long long int arr[1000005],n,a,b,q;
    char s;
    cin>>n>>q;
